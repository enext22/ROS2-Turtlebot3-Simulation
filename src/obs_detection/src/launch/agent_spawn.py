...
def main():
    ...
    parser.add_argument('-urdf', '--agent_urdf', type=str, default='dummy.urdf',
                        help='Name of the agent to spawn')
    parser.add_argument('-n', '--agent_name', type=str, default='dummy_agent',
                        help='Name of the agent to spawn')
    parser.add_argument('-ns', '--agent_namespace', type=str, default='dummy_agent_ns',
    ...
    #     get_package_share_directory('amazon_agent_gazebo'), 'models',
    #     'amazon_agent2', 'model.sdf')
    urdf_file_path = args.agent_urdf
    print(urdf_file_path)

    # We need to remap the transform (/tf) topic so each agent has its own.
    ...
    # remapping rule, i.e. /tf -> /<agent_id>/tf
    ...
    if args.namespace is True:
        node.get_logger().info('spawning `{}` on namespace `{}` at {}, {}, {}'.format(
            args.agent_name, args.agent_namespace, args.x, args.y, args.z))

        request.agent_namespace = args.agent_namespace
        print(args.agent_namespace)

    else:
        node.get_logger().info('spawning `{}` at {}, {}, {}'.format(
            args.agent_name, args.x, args.y, args.z))

    node.get_logger().info('Spawning agent using service: `/spawn_entity`')
